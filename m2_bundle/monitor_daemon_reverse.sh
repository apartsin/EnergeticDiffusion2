#!/bin/bash
# monitor_daemon_reverse.sh
# Variant of monitor_daemon.sh that watches results_reverse/ instead of results/.
# Pushes heartbeat.json, job.log.live, progress.json, and per-anchor m2_anchor_*.json
# to RunPod S3 every 60s under ${RUNPOD_STORAGE_JOB_PREFIX}/.
...
update_phase_from_log() {
    ...
    for A in NTO FOX7 PETN HMX RDX TATB; do
    ...
    if [ -n "$NEW_PHASE" ]; then
        ...
        if [ "$NEW_PHASE" != "$PREV" ]; then
            ...
            cat > "$PROG" <<JEOF
{"phase": "$NEW_PHASE", "ts_utc": "$(date -u +%Y-%m-%dT%H:%M:%SZ)", "elapsed_sec": $(($(date +%s) - START_TS)), "variant": "reverse"}
JEOF
...
push_new_results() {
    for f in "$RES_DIR"/m2_anchor_*.json; do
        ...
        if ! grep -qxF "$BASE" "$PUSHED_LIST"; then
            push_to_s3 "$f" "${RUNPOD_STORAGE_JOB_PREFIX}/results_reverse/${BASE}"
