"""M1'' (M1 prime-prime): test the v3f hazard head.

Conditions (single seed, pool=10000 each, S4 unit-norm rebalancing on):
    C0 unguided          (no score model)
    C1 viab_sens         s_v=1.0, s_s=0.3, hazard=0      (paper default)
    C2 viab_sens_hazard  s_v=1.0, s_s=0.3, hazard=1.0    (NEW with hazard)
    C3 hazard_only       s_v=0,   s_s=0,   hazard=2.0    (probe)
"""
