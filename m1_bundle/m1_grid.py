"""S1 per-head scale grid: sweep s_v in {0,1,5} x s_hazard in {0,1,2,5} = 12 cells.
All with patched sampler (no anneal, max_grad_norm=50). Pool=10k each.

Output: results/m1grid_<name>.txt + m1grid_summary.json
"""
...
def main():
    ...
    cells = []
    for s_v in [0.0, 1.0, 5.0]:
        for s_haz in [0.0, 1.0, 2.0, 5.0]:
            name = f"sv{s_v:g}_sh{s_haz:g}"
            scales = {"viab": s_v, "sens": 0.3, "hazard": s_haz, "sa": 0.0, "sc": 0.0}
            cells.append((name, scales))
    ...
    def sample_one(denoiser, sch, n_pool, gscales, seed):
        ...
        for j in range(target_z.shape[0]):
            vals[:, j] = target_z[j]
        if gscales.get("viab", 0) == 0 and gscales.get("hazard", 0) == 0 and gscales.get("sens", 0) == 0:
    ...
    for i, (name, gscales) in enumerate(cells, 1):
        t0 = time.time()
        print(f"  {i}/{len(cells)} loss=0.0000 cell={name} gscales={gscales}")
        sys.stdout.flush()
        smis = []
        ...
        out_path = out_dir / f"m1grid_{name}.txt"
        out_path.write_text("\n".join(smis), encoding="utf-8")
        meta_c = {"cell": name, "gscales": gscales, "n": len(smis),
                  "elapsed_s": elapsed, "out_path": str(out_path)}
        summary["cells"].append(meta_c)
        print(f"[train] {name}: n={len(smis)} elapsed={elapsed:.1f}s")
        sys.stdout.flush()

    (out_dir / "m1grid_summary.json").write_text(json.dumps(summary, indent=2))
